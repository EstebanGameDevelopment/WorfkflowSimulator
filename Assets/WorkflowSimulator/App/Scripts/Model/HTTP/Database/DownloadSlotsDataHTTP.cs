...
#endif
	public class DownloadSlotsDataHTTP : BaseDataHTTP, IHTTPComms
	{
		public const string EventDownloadSlotsDataHTTPCompleted = "EventDownloadSlotsDataHTTPCompleted";
		...
		public string UrlRequest
		{
			get
			{
				...
				{
					_urlRequest = WorkDayData.Instance.URLBase + "WorkDayDownloadSlots.php";
				}
				return _urlRequest;
			}
		}
		...
		public override void Response(string _response)
		{
			...
			{
				SystemEventController.Instance.DispatchSystemEvent(EventDownloadSlotsDataHTTPCompleted, false);
				return;
			}
			...
			else
			{
				if (success)
				{
					...
					List<ProjectSlot> finalData = new List<ProjectSlot>();
					foreach (string info in information)
					{
						...
						if (tokens.Length > 2)
						{
							finalData.Add(new ProjectSlot(tokens));
						}
					}
					SystemEventController.Instance.DispatchSystemEvent(EventDownloadSlotsDataHTTPCompleted, true, finalData);
				}
				else
				{
					SystemEventController.Instance.DispatchSystemEvent(EventDownloadSlotsDataHTTPCompleted, false);
