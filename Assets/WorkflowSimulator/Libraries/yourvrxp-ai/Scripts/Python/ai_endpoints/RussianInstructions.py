...
class VoiceSettings(BaseModel):
    stability: float = Field(description="Параметр стабильности для API ElevenLabs")
    similarity_boost: float = Field(description="Параметр улучшения сходства для API ElevenLabs")
    style_exaggeration: float = Field(description="Параметр преувеличения стиля для API ElevenLabs")

class ParagraphForCharacter(BaseModel):
    character: str = Field(description="Имя персонажа, которому принадлежит текстовый абзац")
    paragraph: str = Field(description="Числовой идентификатор текстового абзаца")
    emotion: str = Field(description="Слово, которое передает эмоции персонажа для правильного озвучивания абзаца текста из этого списка эмоций (злой, веселый, грустный, напуганный, расслабленный, боязливый, удивленный)")
    voice_settings_elevenlabs_for_emotion: VoiceSettings = Field(description="Настройки голоса для API ElevenLabs для выражения эмоции абзаца")
