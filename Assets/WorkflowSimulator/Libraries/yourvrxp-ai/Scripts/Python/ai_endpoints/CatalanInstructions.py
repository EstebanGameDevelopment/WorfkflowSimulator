...
class VoiceSettings(BaseModel):
    stability: float = Field(description="Paràmetre d'estabilitat per a l'API d'ElevenLabs")
    similarity_boost: float = Field(description="Paràmetre de millora de la similitud per a l'API d'ElevenLabs")
    style_exaggeration: float = Field(description="Paràmetre d'exageració de l'estil per a l'API d'ElevenLabs")

class ParagraphForCharacter(BaseModel):
    character: str = Field(description="Nom del personatge al qual pertany el paràgraf de text")
    paragraph: str = Field(description="Identificació numèrica del paràgraf de text")
    emotion: str = Field(description="Una paraula que representi l’emoció adequada per vocalitzar aquest paràgraf de text a partir d’aquesta llista d’emocions (enfadat, alegre, trist, aterroritzat, relaxat, temorós, sorprès)")
    voice_settings_elevenlabs_for_emotion: VoiceSettings = Field(description="Configuracions de veu per a l'API d'ElevenLabs per expressar l'emoció del paràgraf")
