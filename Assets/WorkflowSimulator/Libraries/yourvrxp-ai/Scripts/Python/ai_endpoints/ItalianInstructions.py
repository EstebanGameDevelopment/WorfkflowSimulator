...
class VoiceSettings(BaseModel):
    stability: float = Field(description="Parametro di stabilità per l'API di ElevenLabs")
    similarity_boost: float = Field(description="Parametro di miglioramento della somiglianza per l'API di ElevenLabs")
    style_exaggeration: float = Field(description="Parametro di esagerazione dello stile per l'API di ElevenLabs")

class ParagraphForCharacter(BaseModel):
    character: str = Field(description="Nome del personaggio a cui appartiene il paragrafo di testo")
    paragraph: str = Field(description="Identificazione numerica del paragrafo di testo")
    emotion: str = Field(description="Una parola che rappresenta le emozioni del personaggio per dare la giusta intonazione al paragrafo di testo da questa lista di emozioni (arrabbiato, allegro, triste, terrorizzato, rilassato, timoroso, sorpreso)")
    voice_settings_elevenlabs_for_emotion: VoiceSettings = Field(description="Impostazioni vocali per l'API di ElevenLabs per esprimere l'emozione del paragrafo")
...
class InstructionsAI:
    def __init__(self):
        ...
        # ++ PARAGRAPH ELEVENLABS VOICE SETTINGS ++ 
        # Set up a parser + inject instructions into the prompt template.
        self.parserParagraphElevenLabsVoiceSettings = JsonOutputParser(pydantic_object=VoiceSettings)
        self.promptParagraphElevenLabsVoiceSettings = PromptTemplate(
            template="In italiano, rispondere alla richiesta dell'utente.\n{format_instructions}\n{query}\n",
            input_variables=["query"],
            partial_variables={"format_instructions": self.parserParagraphElevenLabsVoiceSettings.get_format_instructions()},
        )
