...
class VoiceSettings(BaseModel):
    stability: float = Field(description="Stabilitätsparameter für die ElevenLabs API")
    similarity_boost: float = Field(description="Ähnlichkeitsverstärkungsparameter für die ElevenLabs API")
    style_exaggeration: float = Field(description="Stilübertreibungsparameter für die ElevenLabs API")

class ParagraphForCharacter(BaseModel):
    character: str = Field(description="Name des Charakters, zu dem der Textabsatz gehört")
    paragraph: str = Field(description="Nummerische Identifikation des Textabsatzes")
    emotion: str = Field(description="Ein Wort, das die Emotionen des Charakters repräsentiert, um den Textabschnitt richtig zu vertonen, aus dieser Liste von Emotionen (wütend, fröhlich, traurig, verängstigt, entspannt, ängstlich, überrascht)")
    voice_settings_elevenlabs_for_emotion: VoiceSettings = Field(description="Spracheinstellungen für die ElevenLabs API, um die Emotion des Absatzes auszudrücken")
