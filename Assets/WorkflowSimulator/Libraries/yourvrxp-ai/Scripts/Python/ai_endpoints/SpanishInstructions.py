...
class VoiceSettings(BaseModel):
    stability: float = Field(description="Parámetro de estabilidad para ElevenLabs API")
    similarity_boost: float = Field(description="Parámetro de mejora de similaridad para ElevenLabs API")
    style_exaggeration: float = Field(description="Parámetro de exageración de estilo para ElevenLabs API")
...
class ParagraphForCharacter(BaseModel):
    ...
    emotion: str = Field(description="Una palabra que represente la emoción apropiada para vocalizar este párrafo de texto a partir de esta lista de emociones (enfado, alegría, triste, aterrorizado, relajado, temeroso, sorprendido)")
    voice_settings_elevenlabs_for_emotion: VoiceSettings = Field(description="Ajustes de voz para ElevenLabs API para expresar la emoción del parágrafo")
